 ...
 */
 ...
 func main() {
				 ...
				 flag.Parse()
				 for _, flg := range [4]*string{inputHtmlFile, outFile, accessRequirement, googlePublicKeyUrl} {
					 if *flg == "" {
						 log.Fatal("Missing flag: ", flg)
					 }
				 }
				 ...
				 encryptedDoc, err := encryptionutils.GenerateEncryptedDocument(string(b), *googlePublicKeyUrl, *accessRequirement)
				 if err != nil {
								 log.Fatal(err)
				 }
