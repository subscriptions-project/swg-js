...
const requestOptions = {
  headers: {
    'Access-Control-Allow-Origin': '*',  // TODO(dparikh): Fix "*".
...
export function isSubscriber() {
  return fetch(allowedJsonUrl, requestOptions).then((response) => {
    if (!response.ok) {
...
export function getOffers(offerAccess) {
  ...
  return fetch(offerJsonUrl, requestOptions).then((response) => {
    if (!response.ok) {
