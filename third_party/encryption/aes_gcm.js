...
export function decryptAesGcm(key, text) {
  ...
  return wrapCryptoOp(
    subtle.importKey('raw', keybytes.buffer, 'AES-GCM', true, ['decrypt'])
  ).then(formattedkey => {
      ...
      subtle.decrypt(
        {
          ...
          tag: isIE
            ? bytesToDecrypt.slice(bytesToDecrypt.byteLength - 16)
            : undefined,
        ...
        isIE
          ? bytesToDecrypt.slice(0, bytesToDecrypt.byteLength - 16)
          : bytesToDecrypt
      )
    ).then(buffer => {
